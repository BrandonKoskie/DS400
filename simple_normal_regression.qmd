...
```{r}
ggplot(bikes, aes(x = temp_feel, y = rides, color = season)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Temperature vs. Ridership by Season") +
  theme_minimal()
...
bikes_daily <- bikes %>%
  group_by(date) %>%
  summarize(avg_temp = mean(temp_feel), total_rides = sum(rides))

ggplot(bikes_daily, aes(x = avg_temp, y = total_rides)) +
  geom_line(color = "tomato") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  labs(title = "Average Temperature vs. Total Daily Riders",
