...
fake_news %>%
  ...
  ggplot(aes(x = type, y = n, fill = title_has_excl)) +
  geom_col(position = "fill") +
  labs(
    title = "Proportion of Titles with Exclamation Marks by News Type",
    x = "News Type",
    y = "Proportion",
...
P(B\|A) - what is the probability this person went to the beach given that the are on vacation
...
```{r}
table(pop_vs_soda$region)
prop.table(table(pop_vs_soda$region))
```
...
```{r}
table(pop_vs_soda$region, pop_vs_soda$response)
...
# Compute % of "pop" per region
pop_props <- pop_vs_soda %>%
  group_by(region, word_for_cola) %>%
...
# Define regions & states
state_regions <- tibble(
  region = c("South", "Midwest", "Northeast", "West"),
  states = list(
    c("texas","florida","georgia","alabama","mississippi","louisiana","arkansas",
      "tennessee","kentucky","virginia","north carolina","south carolina","oklahoma","west virginia"),
    c("ohio","indiana","illinois","michigan","wisconsin","minnesota","iowa","missouri",
      "kansas","nebraska","north dakota","south dakota"),
    c("maine","new hampshire","vermont","massachusetts","rhode island","connecticut",
      "new york","new jersey","pennsylvania"),
    c("california","oregon","washington","nevada","idaho","montana","wyoming","colorado",
      "utah","new mexico","arizona","alaska","hawaii")
  )
)

state_region_map <- state_regions %>%
  unnest(states) %>%
  rename(state = states) %>%
  mutate(region = tolower(region))
...
map_data <- states_map %>%
  ...
  left_join(pop_props, by = "region")

# Plot
ggplot(map_data, aes(long, lat, group = group, fill = prop)) +
  geom_polygon(color = "white") +
  coord_fixed(1.3) +
  scale_fill_gradient(low = "lightblue", high = "darkblue", name = "% saying 'pop'") +
  labs(title = "Percentage of People Who Use the Word 'Pop' by Region") +
  theme_minimal(base_size = 14)
