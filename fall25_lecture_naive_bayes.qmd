...
ggplot(penguins, aes(x = flipper_length_mm, y = bill_length_mm, color = species)) +
  ...
  labs(
    title = "Identifying the Unknown Penguin",
    subtitle = "Bill Length vs Flipper Length by Species",
    x = "Flipper Length (mm)",
    y = "Bill Length (mm)",
    color = "Species"
  ) +
...
```{r}
ggplot(pulse_of_the_nation, aes(x = income, fill = climate_change)) +
...
```{r}
climate_model <- naiveBayes(climate_change ~ ., data = pulse_of_the_nation)
```
...
pulse_of_the_nation <- pulse_of_the_nation %>% 
  mutate(predicted_climate = predict(climate_model, newdata = .))
```

```{r}
pulse_of_the_nation %>% 
  tabyl(climate_change,predicted_climate) %>% 
  adorn_percentages("row") %>% 
  adorn_pct_formatting(digits = 2) %>% 
  adorn_ns
```
...
```{r}
our_respondent <- data.frame(books = 10, tranformers = 7, above_average = "under_average")
```

```{r}
climate_model_2 <- naiveBayes(party ~ books + transformers, data = pulse_of_the_nation)
```
...
```{r}
predict(climate_model_2, newdata = our_respondent, type = "raw")
```
