...
```{r}
plot_gamma(shape = 4, rate = 7, mean = TRUE, mode = TRUE) 
```
...
```{r}
plot_gamma(shape = 10.235, rate = 1.847, mean = TRUE, mode = TRUE) 
```
...
1.  Why is the Poisson model a reasonable choice for our data Yi?
    ...
    -   This generally fits our prior understanding of how many goals can be scored in a soccer game, usually very few.
